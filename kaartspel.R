...
{
    lower <- card1$rank > mean(pak$rank)
    higher <- card1$rank < mean(pak$rank)
    ...
    stap <- 2
    correctL <- (lower && card1$rank > card2$rank)
    correctH <- (higher && card1$rank < card2$rank)
